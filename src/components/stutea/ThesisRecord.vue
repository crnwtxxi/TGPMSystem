<template>
    <div class="thesisrecord_container">
        <el-card>
            <div slot="header" style="text-align: left;"><b>我的论文</b></div>
            <el-table :data="tableDataPass" style="width: 100%">
                <el-table-column prop="thesisTitle" label="项目名称" min-width="400"></el-table-column>
                <el-table-column prop="thesisDate" label="结题时间" min-width="200"></el-table-column>
                <el-table-column prop="thesisStatus" label="状态" min-width="150">
        ...
        <el-card style="margin-top: 20px;">
            ...
            <el-table :data="tableDataUnpass" style="width: 100%">
                ...
                <el-table-column prop="thesisStatus" label="状态" min-width="150"
                    ...
                    <template slot-scope="scope">
                        <el-tag
                        :type="scope.row.thesisStatus === '2' ? 'info' : 'warning'"
                        disable-transitions>{{scope.row.thesisStatus === '2' ? '不通过' : '待审核'}}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="操作" min-width="200">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="handleViewThesis(scope.$index, scope.row)">查看详情</el-button>
                        <!-- <el-button size="mini" @click="handleDeleteThesis(scope.$index, scope.row)" type="danger">删除</el-button> -->
                    </template>
                </el-table-column>
            </el-table>
            <div class="page">
...
export default {
    ...
    methods: {
        //删除
        handleDeleteThesis(index, row) {
            console.log(index, row);
        },
        //通过
        handleSizeChangePass(val) {
            this.pageSizePass = val;
            this.getAllPassThesis(this.currentPage, this.pageSize);
        },
        ...
        handleSizeChangeUnpass(val) {
            ...
            this.getAllUpassThesis(this.currentPage, this.pageSize);
        },
        ...
        //标签过滤
        filterTag(value, row) {
            return row.thesisStatus === value;
        },
        //通过
        getAllPassThesis(pageNum, pageSize) {
            this.axios.get('/common/getAllPassThesis/'+pageNum+'/'+pageSize)
            .then(res => {
        ...
        //不通过
        getAllUpassThesis(pageNum, pageSize) {
            this.axios.get('/common/getAllUnpassThesis/'+pageNum+'/'+pageSize)
            .then(res => {
    ...
    mounted() {
        this.getAllPassThesis(this.currentPagePass, this.pageSizePass);
        this.getAllUpassThesis(this.currentPageUnpass, this.pageSizeUnpass);
    }
}
</script>
